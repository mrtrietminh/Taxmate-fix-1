...
import { BookingStep, AccountantProfile, Transaction, HumanMessage } from '../types';
import { formatVND } from '../utils';
import { databaseService } from '../services/databaseService';
import { Check, Send, Sparkles, Copy, Loader2, ArrowLeft, CheckCircle2, FileText } from 'lucide-react';
...
// Hardcoded test accountant UID - must match databaseService.TEST_ACCOUNTANT_UID
const TEST_ACCOUNTANT_UID = 'TEST_ACCOUNTANT_001';
...
const AccountantMatch: React.FC<AccountantMatchProps> = ({ transactions, currentUserUid, currentUserPhone }) => {
    ...
    // Check if already assigned and load messages
    useEffect(() => {
        let isMounted = true;

        const checkStatusAndLoadMessages = async () => {
            try {
                // Check user payment status
                const user = await databaseService.getUser(currentUserUid);
                if (user?.isPaid && isMounted) {
                    setIsAssigned(true);
                    setStep(BookingStep.CONNECTED);
                }

                // Load human chat messages
                if (user?.isPaid || isAssigned) {
                    const roomId = databaseService.generateRoomId(currentUserUid, TEST_ACCOUNTANT_UID);
        ...
        checkStatusAndLoadMessages();
        const interval = setInterval(checkStatusAndLoadMessages, 3000);

        return () => {
            isMounted = false;
            clearInterval(interval);
        };
    }, [currentUserUid, isAssigned]);
    ...
    const handleSendMessage = async () => {
        ...
        const roomId = databaseService.generateRoomId(currentUserUid, TEST_ACCOUNTANT_UID);
        await databaseService.sendHumanMessage(roomId, currentUserUid, 'BUSINESS', chatInput.trim());
    ...
    // Handle payment confirmation - creates client assignment
    const handleConfirmPayment = async () => {
        ...
        try {
            ...
            if (user) {
                ...
                // Create client assignment to the test accountant
                await databaseService.assignClientToAccountant(currentUserUid, TEST_ACCOUNTANT_UID);
                setIsAssigned(true);
    ...
    if (step === BookingStep.MATCHING_CHECKLIST) {
        return (
            <div className="h-full flex flex-col items-center justify-center bg-white p-8 text-center animate-in fade-in zoom-in-95 duration-500">
                ...
                <h2 className="text-xl font-bold text-slate-900 mb-2">Đang kết nối với kế toán...</h2>
                <p className="text-slate-500 text-sm">Chúng tôi đang phân tích ngành hàng và quy mô kinh doanh của bạn.</p>
            </div>
        );
    }

    // SCREEN 4: CONNECTED - HUMAN CHAT
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    ...
                    {messages.length === 0 && (
                        <div className="text-center py-10 text-slate-400 text-sm italic">
                            Bắt đầu cuộc trò chuyện với kế toán của bạn...
                        </div>
                    )}

                    {messages.map((msg, i) => (
                        <div key={msg.id || i} className={`flex ${msg.senderRole === 'BUSINESS' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-[14px] shadow-sm ${msg.senderRole === 'BUSINESS' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white text-slate-800 border border-slate-200 rounded-bl-none'}`}>
                                {msg.text}
                                <p className={`text-[9px] mt-1 text-right ${msg.senderRole === 'BUSINESS' ? 'text-blue-200' : 'text-slate-400'}`}>
